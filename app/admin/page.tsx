    ...
    reader.onload = (event) => {
      try {
        ...
        const cleanData = rawJson.map((row: any) => ({
          ...
          attendance: row["How many LSAs have you attended?\u00a0"] || "Unknown",
          topics: [
          ...
          ].filter(Boolean).join(";") // Combine all topic columns into one string separated by semicolons
        }));
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-slate-800">
      <div className="bg-white p-8 rounded-xl shadow-lg border border-slate-200 max-w-md w-full text-center">
        ...
        <input 
          ...
          className="mb-4 w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-slate-900 file:text-white hover:file:bg-slate-700 cursor-pointer"
        />
